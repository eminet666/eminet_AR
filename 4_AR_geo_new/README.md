## Test AR location

### infos
* [précision](./precision.txt)
* [config GPS](./configuration.txt)

### local (ne marche pas)
* [local arjs](./local/local_test_arjs_webxr.html)
* [local arjs trace](./local/local_test_arjs_webxr_trace.html)
* [local follow pos](./local/local_follow_pos.html)
* [local demo_loc](./local/local_demo_loc.html)

### tools
* [tools gps](./tools/tools_gps.html)
* [tools gps online tool](https://www.coordonnees-gps.fr/ma-position)
* [tools OSM](./tools/osm-my-position/index.html)
* [tools OSM firstpos](./tools/osm-my-position_firstpos/)
* [tools OSM addplaces](./tools/osm-my-position_addplaces/)
* [tools OSM NSEO](./tools/osm-my-position_nseo/)
* [tools url mypos map](https://www.mapsdirections.info/fr/coordonnees-sur-google-map.html)
* [tools url alternative map](https://esri.github.io/html5-geolocation-tool-js/)

### gps
* [gps simple](./gps/gps_simple.html)
* [gps boxes](./gps/gps_boxes.html)
* [gps boxes json spiti](./gps/gps_boxes_json_gps_spiti.html)
* [gps boxes json spiti groupe](./gps/gps_boxes_json_gps_spiti_groupe.html)

### lentas
* [json4](./gps/lentas/gps_boxes_json_gps_lentas.html)

### simulate (désactiver gps sur le tel)
* [simulate simple](./simulate/simulate_simple.html)
* [simulate boxes](./simulate/simulate_boxes.html)
* [simulate distance](./simulate/simulate_distance.html)

### exemples ar.js loc
* [ref aframe ar](https://ar-js-org.github.io/AR.js-Docs/location-based-aframe/)
* [ref exemples](https://github.com/AR-js-org/AR.js/blob/master/aframe/examples/location-based/README.md)
* [ref avoid shaking](./ref/avoid_shaking.html)
* [ref show distances](./ref/show_distance.html)

### tests
* [test AR](./tests/README.md)